...
export function NeuralBridge() {
  const { status, backendUrl, setBackendUrl, checkConnection, isLoggedIn } = useBioStore();
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      {/* Manual Entry only visible when logged in (Admin mode) */}
      {isLoggedIn ? (
        ...
        <div 
          onClick={checkConnection}
          className="p-2 rounded-xl bg-zinc-900/50 border border-zinc-800/50 text-zinc-500 hover:text-emerald-500 transition-all cursor-pointer group"
        >
